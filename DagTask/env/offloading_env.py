'''
@Project ：AC 
@File    ：offloading_env.py
@IDE     ：PyCharm 
@Author  ：chaoyi yang
@Date    ：2023/8/10 23:44 
'''

import numpy as np

from DagTask.env.offloading_task_graph import OffloadingTaskGraph
...
class OffloadingEnvironment():
    ...
    def get_scheduling_cost_step_by_step(self, plan, task_graph):
        ...
        # finish time on cloud for each task #边
            ...
            for task_graph in task_graph_batch:
                ...
                for i in task_graph.prioritize_sequence:
                    ...
                    if len(task_graph.pre_task_sets[i]) != 0:
                        ...
                        # print("task {}, Cloud finish time {}".format(i, FT_cloud[i]))
                        wr_start_time = FT_cloud[i]
                        wr_finish_time = wr_start_time + self.resource_cluster.dl_transmission_cost(
                            task.transmission_data_size)
                        FT_wr[i] = wr_finish_time
                    else:
                        ws_start_time = ws_avaliable_time
                        ws_finish_time = ws_start_time + self.resource_cluster.up_transmission_cost(
                            task.processing_data_size)
                        FT_ws[i] = ws_finish_time
