...
class unload_env(object):
    def __init__(self, edge_process_capable,
                 local_process_capable, bandwidth_up=7.0, bandwidth_dl=7.0):
    ...
    def step(self, file_path, unload_action, task_id):
        ...
        if unload_action == 0:
            ...
            self.FT_local = task_finish_time

            self.current_FT = max(self.FT_local, self.FT_edge)

            # reward2 = behind_time - self.current_FT
            # all_reward2 = bad_time - self.current_FT
            # reward2 = math.exp(behind_time - self.current_FT)
            # reward2 = 1/(self.current_FT -behind_time + 1)
            # mul = 20 * (bad_time - self.current_FT) / bad_time
        ...
        return bad_time, self.current_FT

    # if self.dag_time[0] == 0 and self.dag_time[1] == 0:
    #     if unload_action == 0:
    #         self.FT_local += self.locally_execution_cost(size)
    #     if unload_action == 1:
    #         self.FT_edge += self.mec_execution_cost(size) + self.up_transmission_cost(
    #             size) + self.dl_transmission_cost(size)
    # if self.dag_time[0] == 0 and self.dag_time[1] != 0:
    #     if unload_action == 0:
    #         self.FT_local += self.locally_execution_cost(size)
    #     if unload_action == 1:
    #         self.FT_edge += self.mec_execution_cost(size) + self.up_transmission_cost(size)
